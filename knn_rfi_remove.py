...
def _skelbow(Xd, k):
    ...
    wcss = sum(dist**2)
    return wcss, kmeansvar

def elbow(Xd, max_clusters):
    elbow_results = Parallel(n_jobs=4)(delayed(_skelbow)(Xd, k) for k in range(1, max_clusters))
    tss = sum(pdist(Xd)**2)/Xd.shape[0]
    wcss = np.array([res[0] for res in elbow_results])
    bss = np.array(tss) - wcss
    ncluster = 1
    while bss[ncluster]>1.0001*bss[ncluster-1]:
        ncluster += 1
...
if __name__ == "__main__":
    ...
    for infile in files:
        ...
        for i in xrange(1, ncluster):
            cluster = data_dense.loc[data_dense['cluster']==i]
